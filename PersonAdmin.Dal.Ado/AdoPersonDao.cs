...
namespace PersonAdmin.Dal.Ado
{
    public class AdoPersonDao(IConnectionFactory connectionFactory) : IPersonDao
    {
        ...
        private RowMapper<Person> RowToPersonMapper = (IDataRecord row) =>
                new Person(
                    id: (int)row["Id"],
        ...
        // wenn man die id direkt übergibt gefahrt von sql injections -> besser preparted statements
        public Task<Person?> FindById(int id)
        {
            return template.QuerySingleAsync(
